...
public class miningFWPs {
        ...
        while (i < m && j < n) {
            ...
            if (WNL2.get(j).pre < WNL1.get(i).pre) {
                if (WNL2.get(j).pos > WNL1.get(i).pos) {
                    if (k >= 0) {
                        if (WNL2.get(j).pre == WL3_support.get(k).pre) {
    ...
    public static List<List<String>> FWPODS(SWNNode root, Map<String, String> newT, double minWs, List<TailElement> tail, List<Map<String, String>> window, Map<String, Integer> itemWeights, int originalPreCounter, int originalPostCounter) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= df.size() - windowSize; i++) {
            boolean checkSlidingWindow = false;
            ...
            if(checkSlidingWindow) {
                ...
                System.out.println("Frequent Weighted Patterns:");
                for (List<String> pattern : fwps) {
